...
// Outbox configures the transactional-outbox workers. NATS publish itself
// is driven by an out-of-band CDC bridge (Debezium Server) — see ADR-0018.
// The remaining knob is Retention, which the GC worker uses to decide when
// fully-processed rows are eligible for deletion.
type Outbox struct {
