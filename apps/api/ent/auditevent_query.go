...
type AuditEventQuery struct {
	...
	withOutboxEvent *OutboxEventQuery
...
func (_q *AuditEventQuery) QueryOutboxEvent() *OutboxEventQuery {
...
func (_q *AuditEventQuery) WithOutboxEvent(opts ...func(*OutboxEventQuery)) *AuditEventQuery {
	query := (&OutboxEventClient{config: _q.config}).Query()
...
//		GroupBy(auditevent.FieldCreatedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.AuditEvent.Query().
//		Select(auditevent.FieldCreatedAt).
//		Scan(ctx, &v)
...
func (_q *AuditEventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AuditEvent, error) {
	...
	if query := _q.withOutboxEvent; query != nil {
		if err := _q.loadOutboxEvent(ctx, query, nodes, nil,
			func(n *AuditEvent, e *OutboxEvent) { n.Edges.OutboxEvent = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AuditEventQuery) loadOutboxEvent(ctx context.Context, query *OutboxEventQuery, nodes []*AuditEvent, init func(*AuditEvent), assign func(*AuditEvent, *OutboxEvent)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "outbox_event_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *AuditEventQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withOutboxEvent != nil {
			_spec.Node.AddColumnOnce(auditevent.FieldOutboxEventID)
		}
	}
