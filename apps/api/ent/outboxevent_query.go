...
func (_q *OutboxEventQuery) QueryAuditEvent() *AuditEventQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(outboxevent.Table, outboxevent.FieldID, selector),
...
func (_q *OutboxEventQuery) WithAuditEvent(opts ...func(*AuditEventQuery)) *OutboxEventQuery {
	query := (&AuditEventClient{config: _q.config}).Query()
...
func (_q *OutboxEventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OutboxEvent, error) {
	...
	if query := _q.withAuditEvent; query != nil {
		if err := _q.loadAuditEvent(ctx, query, nodes, nil,
			func(n *OutboxEvent, e *AuditEvent) { n.Edges.AuditEvent = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *OutboxEventQuery) loadAuditEvent(ctx context.Context, query *AuditEventQuery, nodes []*OutboxEvent, init func(*OutboxEvent), assign func(*OutboxEvent, *AuditEvent)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*OutboxEvent)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
		nodeids[nodes[i].ID] = nodes[i]
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(auditevent.FieldOutboxEventID)
	}
	query.Where(predicate.AuditEvent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(outboxevent.AuditEventColumn), fks...))
	}))
