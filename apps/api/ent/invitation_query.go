...
type InvitationQuery struct {
	...
	withDeliveries   *EmailDeliveryQuery
	withFKs          bool
...
func (_q *InvitationQuery) QueryInvitedBy() *UserQuery {
...
func (_q *InvitationQuery) QueryAcceptedBy() *UserQuery {
	query := (&UserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *InvitationQuery) QueryDeliveries() *EmailDeliveryQuery {
	query := (&EmailDeliveryClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *InvitationQuery) WithInvitedBy(opts ...func(*UserQuery)) *InvitationQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *InvitationQuery) WithAcceptedBy(opts ...func(*UserQuery)) *InvitationQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *InvitationQuery) WithDeliveries(opts ...func(*EmailDeliveryQuery)) *InvitationQuery {
	query := (&EmailDeliveryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *InvitationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Invitation, error) {
	...
	if _q.withOrganization != nil || _q.withInvitedBy != nil || _q.withAcceptedBy != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withAcceptedBy; query != nil {
		if err := _q.loadAcceptedBy(ctx, query, nodes, nil,
			func(n *Invitation, e *User) { n.Edges.AcceptedBy = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withDeliveries; query != nil {
		if err := _q.loadDeliveries(ctx, query, nodes,
			func(n *Invitation) { n.Edges.Deliveries = []*EmailDelivery{} },
			func(n *Invitation, e *EmailDelivery) { n.Edges.Deliveries = append(n.Edges.Deliveries, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *InvitationQuery) loadInvitedBy(ctx context.Context, query *UserQuery, nodes []*Invitation, init func(*Invitation), assign func(*Invitation, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Invitation)
	for i := range nodes {
		if nodes[i].user_invitations_sent == nil {
			continue
		}
		fk := *nodes[i].user_invitations_sent
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_invitations_sent" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *InvitationQuery) loadAcceptedBy(ctx context.Context, query *UserQuery, nodes []*Invitation, init func(*Invitation), assign func(*Invitation, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Invitation)
	for i := range nodes {
		if nodes[i].user_invitations_accepted == nil {
			continue
		}
		fk := *nodes[i].user_invitations_accepted
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_invitations_accepted" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *InvitationQuery) loadDeliveries(ctx context.Context, query *EmailDeliveryQuery, nodes []*Invitation, init func(*Invitation), assign func(*Invitation, *EmailDelivery)) error {
	...
	query.withFKs = true
	query.Where(predicate.EmailDelivery(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(invitation.DeliveriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.invitation_deliveries
		if fk == nil {
			return fmt.Errorf(`foreign-key "invitation_deliveries" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "invitation_deliveries" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
